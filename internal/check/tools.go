...
// одиночный исполняемый файл с GitHub releases. Имя файла для текущей
// платформы задаёт адаптер доставки toolchain.
func biomeLinter() toolchain.Executable {
	const version = "2.5.9"
	binary := toolchain.CurrentDelivery().BiomeAsset()
	url := fmt.Sprintf("https://github.com/biomejs/biome/releases/download/@biomejs/biome@%s/%s", version, binary)
...
// внутри архива — папка с бинарём. Целевую платформу для текущей ОС задаёт
// адаптер доставки toolchain.
func ruffLinter() toolchain.Executable {
	const version = "0.16.3"
	target := toolchain.CurrentDelivery().RuffTarget()
	binary := "ruff-" + target + "/ruff"
	url := fmt.Sprintf("https://github.com/astral-sh/ruff/releases/download/%s/ruff-%s.tar.gz", version, target)
